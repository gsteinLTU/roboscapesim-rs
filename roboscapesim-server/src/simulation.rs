...
pub struct Simulation {
    ...
    pub broad_phase: Arc<Mutex<BroadPhaseBvh>>,
...
//            integration_parameters: Arc::new(RwLock::new(IntegrationParameters { max_ccd_substeps: 2, max_stabilization_iterations: 6, max_velocity_friction_iterations: 10, max_velocity_iterations: 14, allowed_linear_error: 0.002, prediction_distance: 0.0015, min_island_size: 64, ..Default::default() })),
            integration_parameters: Arc::new(RwLock::new(IntegrationParameters { max_ccd_substeps: 2, ..Default::default() })),
            physics_pipeline: Arc::new(Mutex::new(PhysicsPipeline::new())),
            island_manager: Arc::new(Mutex::new(IslandManager::new())),
            broad_phase: Arc::new(Mutex::new(BroadPhaseBvh::new())),
    ...
    /// Execute a query operation using a temporary QueryPipeline
    /// The closure receives the QueryPipeline and can perform queries on it
    pub fn with_query_pipeline<F, R>(&self, filter: Option<QueryFilter>, f: F) -> R
    where
        F: FnOnce(&QueryPipeline) -> R,
    {
        let rigid_body_set = self.rigid_body_set.read().unwrap();
        let collider_set = self.collider_set.read().unwrap();
        let broad_phase = self.broad_phase.lock().unwrap();
        let narrow_phase = self.narrow_phase.lock().unwrap();
